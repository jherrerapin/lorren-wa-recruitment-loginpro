...
async function confirmAssignmentFromInboundMessage(activeClient, message, eventName = 'message') {
  ...
  if (!assignment) {
    console.warn(`[dispatch-wa] Se recibió Confirmado desde ${phone}, pero no hay asignación pendiente asociada. event=${eventName}`);
    return false;
  }
  ...
  console.log(`[dispatch-wa] Confirmación automática registrada para assignment=${assignment.id} phone=${phone} event=${eventName}.`);
  return true;
}
