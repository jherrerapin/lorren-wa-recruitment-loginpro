...
// Compatibilidad únicamente para candidatos que quedaron en estos modos antes de #1685.
// Ya no implican que un archivo deba reenviarse ni que haya sido ocultado/no persistido.
const LEGACY_CAMPAIGN_CONFIRMATION_CV_MODE = `${CAMPAIGN_VACANCY_CONFIRMATION_MODE}:cv_resend`;
const LEGACY_PRE_CONSENT_CV_RESEND_MODE = 'pre_consent_cv_resend';
...
const CONSENT_PROMPT = process.env.DATA_CONSENT_PROMPT || `Para continuar con tu postulación, necesito tu autorización para tratar tus datos personales, hoja de vida y documentos con fines de reclutamiento de LoginPro.\n\n${DATA_CONSENT_TEXT}\n\nElige Sí autorizo o No autorizo. También puedes responder por escrito.`;
const CONSENT_CLARIFIER_REPLY = 'Necesito saber si autorizas a LoginPro a tratar tus datos y hoja de vida para este proceso. Elige Sí autorizo o No autorizo. También puedes responder por escrito.';
...
function isAwaitingCampaignVacancyConfirmation(candidate = {}) {
  return Boolean(
    candidate?.vacancyId
    && [CAMPAIGN_VACANCY_CONFIRMATION_MODE, LEGACY_CAMPAIGN_CONFIRMATION_CV_MODE].includes(String(candidate?.botResumeMode || ''))
  );
}
...
function isContextualInterestConfirmationMode(mode = '') {
  ...
  return value === APPLICATION_INTEREST_PENDING_MODE
    || value === LEGACY_PRE_CONSENT_CV_RESEND_MODE
...
export function buildConsentPendingMode({ resumeMode = null } = {}) {
  const normalizedResumeMode = String(resumeMode || '').trim() || null;
  if (!normalizedResumeMode) return DATA_CONSENT_PENDING_MODE;
  const encoded = Buffer.from(JSON.stringify({ resumeMode: normalizedResumeMode }), 'utf8').toString('base64url');
...
export function parseConsentPendingMode(mode = '') {
  ...
  if (value === DATA_CONSENT_PENDING_MODE) {
    return { pending: true, resumeMode: null };
  }
  if (!value.startsWith(CONSENT_PENDING_CONTEXT_PREFIX)) {
    return { pending: false, resumeMode: null };
  }
  try {
...
export function evaluateConsentBoundary(candidate = {}, message = {}) {
  ...
  const pendingContext = parseConsentPendingMode(candidate?.botResumeMode);
  const consentButton = String(message.interactive?.button_reply?.id || '').startsWith('data_consent:');
  const explicitPendingDecision = pendingContext.pending && (
    consentButton
    || shouldRecordConsentAcceptance(body, { consentPromptPending: true })
    || shouldRecordConsentRejection(body, { consentPromptPending: true })
    || Boolean(buildConsentQuestionReply(body))
  );

  if (candidate?.dataConsentStatus === 'REVOKED') {
    if (isAttachmentMessage(message)) return { block: false, reason: 'consent_revoked_attachment_passthrough' };
    return { block: true, reason: 'consent_revoked' };
  }

  if (candidate?.dataConsentStatus === 'ACCEPTED' && !isConsentAlreadyAccepted(candidate)) {
    if (isAttachmentMessage(message)) return { block: false, reason: 'consent_version_required_attachment_passthrough' };
    if (consentButton || hasExplicitConsentAcceptance(body) || hasExplicitConsentRejection(body)) {
      return { block: true, reason: 'consent_version_required' };
    }
  }

  if (pendingContext.pending) {
    if (explicitPendingDecision) return { block: true, reason: 'consent_pending_decision' };
    return { block: false, reason: 'consent_pending_passthrough' };
  }
...
function consentEvidenceBody(body = '', message = {}) {
  ...
  if (isAttachmentMessage(message)) return '(archivo adjunto)';
  return '';
}
...
export function buildConsentAcceptedReply(candidate = {}, vacancy = null) {
  const readiness = getCandidateReadiness(candidate, vacancy, { requireCv: true });
  return ['Gracias, tu autorización quedó registrada.', buildMissingFieldReply(readiness)]
    .filter(Boolean)
    .join(' ');
}

export function deriveConsentResumeUpdate(resumeMode = null) {
  const normalizedResumeMode = String(resumeMode || '') === LEGACY_PRE_CONSENT_CV_RESEND_MODE
    ? null
    : resumeMode;
  const alternative = parseAlternativeMode(normalizedResumeMode);
  ...
  const captureMode = PRE_CONSENT_OFFER_TO_CAPTURE_MODE.get(String(normalizedResumeMode || ''));
  if (captureMode) {
    return {
      currentStep: ConversationStep.COLLECTING_DATA,
  ...
  if (isPreConsentCaptureMode(normalizedResumeMode)) {
    return {
      currentStep: ConversationStep.COLLECTING_DATA,
      botResumeMode: normalizedResumeMode
    };
  }
  return {
    currentStep: ConversationStep.COLLECTING_DATA,
...
export async function resolveConsentResumeContext(prisma, resumeMode = null) {
  ...
  if (!alternative.active || !alternative.vacancyId) {
    return {
      resumeUpdate: deriveConsentResumeUpdate(normalizedResumeMode),
      vacancy: null,
  ...
  if (isAffirmativeVacancyConfirmation(body)) {
    ...
    if (explicitApplicationInterest) {
      ...
      await requestDataConsent(prisma, { candidate, to: from, inboundMessageId: message.id,
        candidateUpdates: { currentStep: ConversationStep.GREETING_SENT } });
      return true;
    }
...
function getConsentContext(candidate = {}) {
  ...
  const mode = String(candidate.botResumeMode || '');
  if ([APPLICATION_INTEREST_PENDING_MODE, LEGACY_PRE_CONSENT_CV_RESEND_MODE].includes(mode)) {
    return { pending: false, resumeMode: null };
  }
  return {
    pending: false,
...
async function handleConsentDecision(prisma, req, candidate, message, from, body, boundaryReason) {
  ...
  if (candidate?.dataConsentStatus === 'REVOKED') {
    // Existing REVOKED remains terminal for ordinary turns. Only a new,
  ...
  if (candidate.dataConsentStatus === 'ACCEPTED' && !isConsentAlreadyAccepted(candidate)
    ...
    // Data storage is no longer blocked by consent, but the current legal version
    // still needs its own explicit acceptance event.
  ...
  if (context.pending && shouldRecordConsentAcceptance(body, { consentPromptPending: true })) {
    ...
    const reply = [questionReply, buildConsentAcceptedReply(acceptedCandidate, vacancy)]
      .filter(Boolean)
      .join('\n\n');
  ...
  if (context.pending) {
    ...
    if (!claimed) return true;
    if (!questionReply) {
      // Only actual consent turns arrive here. A failed legal prompt may be
      // recovered, while ordinary data/files pass through to the webhook.
      await requestDataConsent(prisma, { candidate, to: from, inboundMessageId: message.id, context });
      return true;
    }
...
async function preparePausedCandidateForConsentGate(_prisma, candidate = {}) {
  if (!candidate?.botPaused) {
    return { candidate, blocked: false, consume: false, defer: false, retry: false, reason: 'automation_not_paused' };
  }
  // La pausa humana ya no convierte datos o adjuntos en contenido protegido.
  // Se difieren al webhook, que conserva el inbound y sus archivos con las
  // mismas autoridades de persistencia usadas en cualquier otro turno.
  return {
    candidate,
    blocked: false,
    consume: false,
  ...
  return async (req, res, next) => {
    ...
    try {
      for (const message of messages) {
        ...
        const pendingContext = parseConsentPendingMode(candidate?.botResumeMode);
        const consentButton = String(message.interactive?.button_reply?.id || '').startsWith('data_consent:');
        const explicitConsentTurn = consentButton
          || shouldRecordConsentAcceptance(body, { consentPromptPending: pendingContext.pending })
          || shouldRecordConsentRejection(body, { consentPromptPending: pendingContext.pending })
          || Boolean(buildConsentQuestionReply(body));
        ...
        if (!withdrawalRequested && !explicitConsentTurn) {
        ...
        try {
          ...
          const handled = campaign && !withdrawalRequested
            ...
            : await handleConsentDecision(prisma, req, candidate, message, from, body, boundary.reason);
          if (handled) {
