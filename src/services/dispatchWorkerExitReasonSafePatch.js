...
function canUseOps(req = {}) { const role = req.session?.userRole || req.userRole; const username = clean(req.session?.username || req.username) || ''; return role === 'dev' || Boolean(req.session?.canAccessDispatch || req.canAccessDispatch) || username.startsWith('operaciones-despacho'); }
function redirectBack(req, message) { const referer = req.get?.('referer') || '/admin/operaciones/personal'; const separator = referer.includes('?') ? '&' : '?'; return `${referer}${separator}message=${encodeURIComponent(message)}`; }
function todayCO() { return new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Bogota', year: 'numeric', month: '2-digit', day: '2-digit' }).format(new Date()); }
function normalizeDateParam(value, fallback = todayCO()) { const raw = clean(value); return raw && /^\d{4}-\d{2}-\d{2}$/.test(raw) ? raw : fallback; }
function selectedDateFromRequest(req = {}) { return normalizeDateParam(req.query?.fecha || req.query?.date); }
function dayRange(dateText) { const start = new Date(`${dateText}T00:00:00.000Z`); const end = new Date(start); end.setUTCDate(end.getUTCDate() + 1); return { start, end }; }
function reasonNote(reason, note) { return `${NOTE_PREFIX} ${reason.label}${note ? ` | Nota: ${note}` : ''}`; }
function extractReasonFromNotes(notes) { const text = String(notes || ''); const index = text.lastIndexOf(NOTE_PREFIX); if (index < 0) return null; const raw = text.slice(index + NOTE_PREFIX.length).split('\n')[0].trim(); return raw.split('|')[0].trim() || null; }
function buildOperationalCityFilter(operationalCityId) { return operationalCityId ? { cities: { some: { cityId: operationalCityId } } } : {}; }
function buildWorkerSearchFilter(q) { return q ? { OR: [{ fullName: { contains: q, mode: 'insensitive' } }, { documentNumber: { contains: q, mode: 'insensitive' } }, { phone: { contains: q, mode: 'insensitive' } }] } : {}; }
function cleanStrings(rows, field) { return [...new Set(rows.map((row) => clean(row?.[field])).filter(Boolean))].sort((a, b) => a.localeCompare(b, 'es')); }
...
async function cancelActiveAssignments(prismaClient, workerId, label) {
  ...
  await prismaClient.$transaction(assignments.map((item) => prismaClient.dispatchAssignment.update({ where: { id: item.id }, data: { status: item.status === CONFIRMED_ASSIGNMENT_STATUS ? 'NO_CONFIRMO' : 'CANCELLED', notes: `Auxiliar desactivado desde personal. Causal: ${label}.` } })));
...
async function handleToggle(req, res, next, workerId) {
  ...
  } catch (error) { console.error('[dispatch-worker-toggle-safe]', error); return next(error); }
}
...
async function stats(workerIds = []) {
  ...
  const count = new Map(); const workers = {};
  for (const row of rows) { const label = extractReasonFromNotes(row.notes); if (!label) continue; count.set(label, (count.get(label) || 0) + 1); if (workerIds.includes(row.id)) workers[row.id] = { label }; }
...
async function buildDashboardMetrics(prismaClient, selectedDate) {
  const { start, end } = dayRange(selectedDate); const whereForDate = { serviceDate: { gte: start, lt: end } };
...
async function handleOperationsDashboard(req, res, next) {
  try {
    if (!canUseOps(req)) return res.status(403).send('Modulo no habilitado para este usuario');
    const selectedDate = selectedDateFromRequest(req); const metrics = await buildDashboardMetrics(db(), selectedDate);
  ...
  } catch (error) { console.error('[dispatch-dashboard-date-safe]', error); return next(error); }
}

async function handleAssignmentsBoard(req, res, next) {
  try {
    if (!canUseOps(req)) return res.status(403).send('Modulo no habilitado para este usuario');
    const prismaClient = db(); const selectedDate = selectedDateFromRequest(req); const { start, end } = dayRange(selectedDate);
    const q = clean(req.query?.q); const operationalCityId = clean(req.query?.operationalCityId); const transportMode = clean(req.query?.transportMode); const locality = clean(req.query?.locality); const serviceRequestId = clean(req.query?.serviceRequestId);
    ...
    return res.render('operacionesAsignacionesConfirmacion', { activeStatuses: ACTIVE_ASSIGNMENT_STATUSES, workers, availableWorkers, assignedWorkerIdsOnSelectedDate: new Set(sameDayAssignments.map((assignment) => assignment.workerId)), cities, serviceRequests, selectedServiceRequest, selectedServiceRequestId: selectedServiceRequest?.id || '', clients, message: clean(req.query?.message), filters: { q: q || '', operationalCityId: operationalCityId || '', transportMode: transportMode || '', locality: locality || '', fecha: selectedDate }, transportModes: cleanStrings(transportModeRows, 'transportMode'), localities: cleanStrings(localityRows, 'residenceLocality'), role: req.session?.userRole || req.userRole, canAccessDispatch: Boolean(req.session?.canAccessDispatch || req.canAccessDispatch) });
  } catch (error) { console.error('[dispatch-assignments-date-safe]', error); return next(error); }
}
...
function installRouterPatch() {
  ...
  express.Router = function patchedRouter(...args) {
    ...
    router.use(async (req, res, next) => {
      ...
      if (req.method === 'GET' && pathname === `${BASE_PATH}/personal/exit-reasons`) { if (!canUseOps(req)) return res.status(403).json({ ok: false }); const workerIds = String(req.query?.workerIds || '').split(',').map((id) => id.trim()).filter(Boolean).slice(0, 500); return res.json({ ok: true, reasons: EXIT_REASONS, ...(await stats(workerIds)) }); }
      return next();
    });
    return router;
  };
...
function uiScript() { return `<script ${MARKER}>(function(){var reasons=${JSON.stringify(EXIT_REASONS)};function h(v){return String(v||'').replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c];});}function n(v){return String(v||'').trim().toLowerCase();}function options(){return reasons.map(function(r){return '<option value="'+h(r.code)+'">'+h(r.label)+'</option>';}).join('');}var selectedForm=null;function ensureModal(){var m=document.getElementById('exit-reason-modal');if(m)return m;m=document.createElement('div');m.className='modal-overlay';m.id='exit-reason-modal';m.innerHTML='<div class="modal"><h3>Desactivar auxiliar</h3><p>Selecciona la causal de retiro.</p><div class="field" style="margin-bottom:12px"><label>Causal de retiro *</label><select id="exitReasonCode"><option value="">Selecciona una causal</option>'+options()+'</select></div><div class="field" style="margin-bottom:16px"><label>Nota opcional</label><textarea id="exitReasonNote" rows="3" style="width:100%;border:1px solid var(--border);border-radius:10px;padding:10px 12px;font-family:inherit"></textarea></div><div class="modal-actions"><button type="button" class="btn btn-secondary" id="exitReasonCancel">Cancelar</button><button type="button" class="btn btn-danger" id="exitReasonConfirm">Desactivar auxiliar</button></div></div>';document.body.appendChild(m);document.getElementById('exitReasonCancel').onclick=function(){m.classList.remove('open')};return m;}document.querySelectorAll('form[action*="/admin/operaciones/personal/"][action$="/toggle"]').forEach(function(form){var button=form.querySelector('button[type="submit"]');if(!button||n(button.textContent)!=='desactivar')return;form.addEventListener('submit',function(e){e.preventDefault();selectedForm=form;var m=ensureModal();document.getElementById('exitReasonCode').value='';document.getElementById('exitReasonNote').value='';m.classList.add('open');});});document.addEventListener('click',function(e){if(!e.target||e.target.id!=='exitReasonConfirm')return;var code=document.getElementById('exitReasonCode');var note=document.getElementById('exitReasonNote');if(!code.value){code.focus();return;}selectedForm.querySelectorAll('input[name="exitReasonCode"],input[name="exitReasonNote"]').forEach(function(input){input.remove()});var a=document.createElement('input');a.type='hidden';a.name='exitReasonCode';a.value=code.value;var b=document.createElement('input');b.type='hidden';b.name='exitReasonNote';b.value=note.value||'';selectedForm.appendChild(a);selectedForm.appendChild(b);selectedForm.submit();});})();</script>`; }

function assignmentDateScript(req) {
  const selectedDate = selectedDateFromRequest(req);
  return `<script data-dispatch-date-filter-safe="true">(function(){var selectedDate=${JSON.stringify(selectedDate)};document.querySelectorAll('a[href="/admin/operaciones/solicitudes"]').forEach(function(a){if((a.textContent||'').toLowerCase().indexOf('solicitudes')>=0)a.textContent='Crear solicitud';});var requestPanel=[].slice.call(document.querySelectorAll('.board-panel-head h2')).find(function(h){return /solicitudes/i.test(h.textContent||'');});if(requestPanel&&!document.getElementById('assignmentDateFilter')){var form=document.createElement('form');form.id='assignmentDateFilter';form.method='get';form.action='/admin/operaciones/asignaciones';form.className='filters';form.style.borderBottom='1px solid #edf2f7';form.innerHTML='<div class="field"><label>Fecha de solicitudes</label><input type="date" name="fecha" value="'+selectedDate+'"></div><button class="btn btn-primary" type="submit">Aplicar fecha</button>';requestPanel.parentElement.insertAdjacentElement('afterend',form);}document.querySelectorAll('a[href^="/admin/operaciones/asignaciones?serviceRequestId="]').forEach(function(link){try{var url=new URL(link.href,window.location.origin);url.searchParams.set('fecha',selectedDate);link.href=url.pathname+'?'+url.searchParams.toString();}catch(e){}});document.querySelectorAll('form[action="/admin/operaciones/asignaciones"]').forEach(function(form){if(!form.querySelector('input[name="fecha"]')){var input=document.createElement('input');input.type='hidden';input.name='fecha';input.value=selectedDate;form.appendChild(input);}});document.querySelectorAll('.request-card .request-actions').forEach(function(actions){if(actions.querySelector('form[data-service-request-delete="true"]'))return;var edit=actions.querySelector('a[href*="/asignaciones/solicitudes/"][href$="/editar"]');if(!edit)return;var match=edit.getAttribute('href').match(/solicitudes\\/([^/]+)\\/editar/);if(!match)return;var f=document.createElement('form');f.method='post';f.action='/admin/operaciones/solicitudes/'+match[1]+'/eliminar';f.setAttribute('data-service-request-delete','true');f.innerHTML='<button class="btn" type="submit">Eliminar</button>';f.addEventListener('submit',function(ev){if(!confirm('¿Eliminar esta solicitud de servicio?'))ev.preventDefault();});actions.appendChild(f);});})();</script>`;
}
function patchWhatsappContext(output) { return output.replace("body:JSON.stringify({phone,message})", "body:JSON.stringify({phone,message,context:{assignmentId:button.closest('.assigned-card')?.dataset.assignmentId||'',serviceRequestId:document.querySelector('input[name=\"serviceRequestId\"]')?.value||new URLSearchParams(window.location.search).get('serviceRequestId')||'',recipientName:button.closest('.assigned-card')?.dataset.workerName||'',messageType:'ASSIGNMENT_CONFIRMATION'}})"); }
function patchDashboardText(output) { return output.replace('>Solicitudes de servicio</a>', '>Crear solicitud</a>').replace('Consulta solicitudes creadas por clientes o internas.', 'Abre solo el formulario para crear una solicitud nueva.'); }
function installUiPatch() {
  if (express.response.__dispatchWorkerExitReasonSafeUi) return;
  const originalSend = express.response.send;
  express.response.send = function patchedSend(body) {
    let output = body; const pathname = pathOf(this.req);
    if (typeof output === 'string' && this.req?.method === 'GET' && (pathname === BASE_PATH || pathname === `${BASE_PATH}/`)) output = patchDashboardText(output);
    if (typeof output === 'string' && this.req?.method === 'GET' && pathname === `${BASE_PATH}/personal` && output.includes('</body>') && !output.includes(MARKER)) output = output.replace('</body>', `${uiScript()}\n</body>`);
    if (typeof output === 'string' && this.req?.method === 'GET' && pathname === `${BASE_PATH}/asignaciones` && output.includes('</body>')) { output = patchWhatsappContext(output); if (!output.includes('data-dispatch-date-filter-safe="true"')) output = output.replace('</body>', `${assignmentDateScript(this.req)}\n</body>`); }
