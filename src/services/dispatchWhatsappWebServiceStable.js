...
const NOT_CONNECTED_MESSAGE = 'WhatsApp de despacho no está conectado. Espera unos segundos y vuelve a intentar.';
const LOGGED_OUT_MESSAGE = 'Sesión de WhatsApp despacho cerrada. Abre la pantalla de WhatsApp para reconectar o escanear QR si aparece.';
...
const INIT_TIMEOUT_MS = Number(process.env.DISPATCH_WWEB_INIT_TIMEOUT_MS || 45000);
...
function normalizeWhatsappSenderId(senderId) {
  return normalizePhone(String(senderId || '').split('@')[0]);
}
...
function killStaleChromiumProcesses(dataPath) {
  ...
  try { execFileSync('sh', ['-c', `pkill -f ${shellQuote(dataPath)} || true`], { stdio: ['ignore', 'ignore', 'ignore'] }); }
  catch (error) { console.warn('No fue posible limpiar procesos Chromium anteriores de WhatsApp despacho.', error); }
}
...
function resolveChromeExecutablePath() {
  ...
  try {
    return execFileSync('sh', ['-c', 'command -v chromium || command -v chromium-browser || command -v google-chrome-stable || command -v google-chrome || find /nix/store -path \'*/bin/chromium\' -type f 2>/dev/null | head -n 1'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim() || undefined;
  } catch (_error) {
    return undefined;
  }
}
...
function buildPuppeteerOptions() {
  ...
  if (!executablePath) {
    lastError = 'No se encontró Chrome/Chromium en el servidor para iniciar la sesión de WhatsApp despacho. Verifica la instalación de Chromium.';
...
function formatBrowserLaunchError(error) {
  ...
  if (rawMessage.includes('The profile appears to be in use') || rawMessage.includes('process_singleton_posix')) return 'El perfil de WhatsApp despacho estaba bloqueado por Chromium. El sistema limpió el lock y reintentará la conexión.';
  if (rawMessage.includes('ENOENT') || rawMessage.includes('Could not find Chrome') || rawMessage.includes('Failed to launch the browser process')) return 'No se encontró Chrome/Chromium en el servidor para iniciar la sesión de WhatsApp despacho.';
  return rawMessage;
}
...
function startInitWatchdog(activeClient) {
  clearInitTimeout();
  initTimeout = setTimeout(() => {
    if (!initializing || ready || lastQr || client !== activeClient) return;
    lastError = 'La conexión de WhatsApp quedó demasiado tiempo preparando. Se reinició automáticamente la instancia.';
    console.warn(lastError);
    resetClientReference();
    scheduleReconnect('timeout de inicialización');
  }, INIT_TIMEOUT_MS);
}
...
async function findLatestPendingAssignmentByPhone(phone) {
  ...
  const candidates = await prisma.dispatchAssignment.findMany({ where: { status: { in: PENDING_ASSIGNMENT_STATUSES }, worker: { phone: { contains: lastTen } } }, include: { worker: true }, orderBy: { updatedAt: 'desc' }, take: 10 });
...
async function confirmAssignmentFromInboundMessage(activeClient, message) {
  ...
  await prisma.dispatchAssignment.update({ where: { id: assignment.id }, data: { status: CONFIRMED_ASSIGNMENT_STATUS } });
...
export function initDispatchWhatsappClient() {
  ...
  loadWhatsappWebModule()
    .then(({ Client: WhatsappClient, LocalAuth: WhatsappLocalAuth }) => {
      ...
      client = new WhatsappClient({ authStrategy: new WhatsappLocalAuth({ clientId: 'dispatch', dataPath }), takeoverOnConflict: true, takeoverTimeoutMs: 0, puppeteer: buildPuppeteerOptions() });
      startInitWatchdog(client);
      client.on('qr', (qr) => { lastQr = qr; initializing = false; ready = false; lastError = null; clearInitTimeout(); console.log('QR de WhatsApp despacho pendiente. Escanéalo para vincular la sesión:'); printTerminalQr(qr); });
      ...
      client.on('ready', () => { initializing = false; ready = true; lastQr = null; lastError = null; lastReadyAt = new Date().toISOString(); clearInitTimeout(); console.log('WhatsApp de despacho conectado.'); });
      client.on('message', (message) => { confirmAssignmentFromInboundMessage(client, message).catch((error) => console.error('[dispatch-wa] Error procesando respuesta entrante.', error)); });
      ...
      client.initialize().catch((error) => { ready = false; lastQr = null; lastError = formatBrowserLaunchError(error); console.error('Error inicializando WhatsApp de despacho.', error); resetClientReference(); scheduleReconnect('error de inicio'); }).finally(() => { if (!ready && !lastQr) initializing = false; clearInitTimeout(); });
      return client;
    })
    .catch((error) => { ready = false; lastQr = null; lastError = error?.message || 'No fue posible preparar WhatsApp de despacho.'; initializing = false; clearInitTimeout(); console.error('Error preparando WhatsApp de despacho.', error); });
  return client;
}
