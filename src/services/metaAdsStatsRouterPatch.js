...
import {
  ...
  campaignMetaStatus,
  missingVacancyCount
} from './metaRecruitmentStats.js';

const BASE_PATH = '/admin/estadisticas';
const DEFAULT_LOOKBACK_DAYS = 90;
let prismaForStats = null;

function db() {
  if (!prismaForStats) prismaForStats = new PrismaClient();
  return prismaForStats;
}
...
function layout(title, body) {
  return `<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><link rel="icon" type="image/svg+xml" href="/public/favicon-loginpro.svg?v=meta-stats">${baseStyles()}</head><body><nav class="nav"><a href="/admin">Panel</a><a href="${BASE_PATH}">Estadísticas</a><a href="${BASE_PATH}/campaigns" class="active">Anuncios</a><span class="spacer"></span><a href="/logout">Cerrar sesión</a></nav><main class="page">${body}</main></body></html>`;
}
...
async function loadDashboard(query = {}) {
  ...
  const [campaigns, candidates, cities, vacancies, account, lastSnapshot] = await Promise.all([
    ...
    db().city.findMany({ where: { usedForRecruitment: true }, orderBy: { name: 'asc' }, select: { name: true } }),
    db().vacancy.findMany({ orderBy: [{ city: 'asc' }, { title: 'asc' }], select: { id: true, title: true, city: true, isActive: true } }),
    db().metaAdAccount.findFirst({ where: { isActive: true }, orderBy: { updatedAt: 'desc' } }),
    db().metaAdSnapshot.findFirst({ orderBy: { updatedAt: 'desc' }, select: { updatedAt: true } })
  ]);

  const snapshots = campaigns.length
    ? await db().metaAdSnapshot.findMany({
...
async function renderList(req, res) {
  ...
  const data = await loadDashboard(req.query || {});
...
async function renderDetail(req, res, campaignId) {
  if (!canAccess(req)) return res.status(403).send('No autorizado.');
  const data = await loadDashboard({ ...req.query, historical: '1' });
  const metric = data.metrics.find((item) => item.campaign.id === campaignId);
...
async function saveClassification(req, res, campaignId) {
  if (!canAccess(req)) return res.status(403).send('No autorizado.');
  const requestedVacancyId = normalizeText(req.body?.vacancyId);
  const requestedCity = normalizeText(req.body?.city);
  const campaign = await db().campaign.findUnique({ where: { id: campaignId }, select: { id: true, code: true } });
  if (!campaign) return res.status(404).send('Anuncio no encontrado.');

  let vacancy = null;
  if (requestedVacancyId) {
    vacancy = await db().vacancy.findUnique({ where: { id: requestedVacancyId }, select: { id: true, city: true } });
    if (!vacancy) return res.status(400).send('La vacante seleccionada no existe.');
  }
  const city = vacancy?.city || requestedCity || null;
  await db().campaign.update({ where: { id: campaignId }, data: { vacancyId: vacancy?.id || null, city } });
  ...
  return res.redirect(`${BASE_PATH}/campaigns/${encodeURIComponent(campaignId)}?success=1`);
}

async function jsonList(req, res) {
...
function installMetaAdsStatsRouterPatch() {
  if (express.__metaAdsStatsRouterPatchInstalled) return;
  ...
  express.Router = function patchedRouter(...args) {
    ...
    router.use(async (req, res, next) => {
      ...
      try {
        if (req.method === 'GET' && path === `${BASE_PATH}/campaigns`) return await renderList(req, res);
        if (req.method === 'GET' && path === `${BASE_PATH}/campaigns.json`) return await jsonList(req, res);
        if (req.method === 'POST' && path === `${BASE_PATH}/campaigns`) return res.redirect(`${BASE_PATH}/campaigns`);
        const editMatch = path.match(/^\/admin\/estadisticas\/campaigns\/([^/]+)\/edit$/);
        if (req.method === 'POST' && editMatch) return await saveClassification(req, res, decodeURIComponent(editMatch[1]));
        const detailMatch = path.match(/^\/admin\/estadisticas\/campaigns\/([^/]+)$/);
        if (req.method === 'GET' && detailMatch) return await renderDetail(req, res, decodeURIComponent(detailMatch[1]));
        return next();
      } catch (error) {
        console.error('[META_ADS_STATS_DASHBOARD_ERROR]', error);
        if (res.headersSent) return next(error);
        return res.status(500).send(layout('Error de estadísticas', '<div class="alert bad">No fue posible cargar las estadísticas. Revisa la sincronización e intenta nuevamente.</div>'));
      }
    });
    return router;
  };
  Object.assign(express.Router, originalRouter);
  express.__metaAdsStatsRouterPatchInstalled = true;
}

installMetaAdsStatsRouterPatch();

export default { installMetaAdsStatsRouterPatch };
