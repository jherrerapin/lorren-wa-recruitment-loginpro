...
export async function processScheduledReminders(prisma, now = new Date()) {
  await runCandidateProcessReminderDispatcher(prisma, { now });
}
...
export async function sendInterviewWindowKeepalive(prisma, candidateId, now = new Date()) {
  ...
  const booking = await findActiveInterviewBooking(prisma, candidateId);
  if (!booking || shouldStopInterviewAutomation(booking, now)) return false;
  if (await hasInterviewKeepaliveSinceLastInbound(prisma, candidate.id, candidate.lastInboundAt)) return false;
  if (!await claimInterviewKeepalive(prisma, candidate, now)) return false;

  const body = buildInterviewKeepaliveText(candidate, booking);
  const windowState = getWhatsappWindowState(candidate.lastInboundAt, now);
  await sendTextMessage(candidate.phone, body);
  await storeOutbound(prisma, candidate.id, body, { source: INTERVIEW_KEEPALIVE_SOURCE, bookingId: booking.id, windowExpiresAt: windowState.expiresAt?.toISOString?.() || null });
  return true;
}
...
export async function sendInterviewBookingReminder(prisma, bookingId, now = new Date()) {
  ...
  const scheduledAt = new Date(booking.scheduledAt);
  const windowStart = new Date(scheduledAt.getTime() - INTERVIEW_REMINDER_LEAD_MS - INTERVIEW_REMINDER_LATE_TOLERANCE_MS);
  const windowEnd = new Date(scheduledAt.getTime() - INTERVIEW_REMINDER_LEAD_MS + INTERVIEW_REMINDER_EARLY_TOLERANCE_MS);
  if (now < windowStart || now > windowEnd) return false;

  if (!await claimInterviewBookingReminder(prisma, booking, now, { windowStart, windowEnd })) return false;

  const vacancy = await findBookingVacancy(prisma, booking, booking.candidate);
  const body = buildInterviewReminderText(booking.candidate, booking, vacancy);
  await sendTextMessage(booking.candidate.phone, body);
  await storeOutbound(prisma, booking.candidate.id, body, { source: INTERVIEW_BOOKING_REMINDER_SOURCE, bookingId: booking.id, scheduledAt: scheduledAt.toISOString() });
  await prisma.candidate.update({
