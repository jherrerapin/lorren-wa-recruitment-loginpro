...
(() => {
  ...
  function removeDetailNoise() {
    document.querySelectorAll('.fact').forEach((fact) => {
      const label = normalized(fact.querySelector('span')?.textContent);
      if (label.includes('inicio de almuerzo') || label.includes('fin de almuerzo')) fact.remove();
    });

    document.querySelectorAll('.risk-flag, .info-chip').forEach((chip) => {
      const text = normalized(chip.textContent);
      if (text.includes('almuerzo') || text.includes('anticipado') || text.includes('hora ordinaria')) chip.remove();
    });
