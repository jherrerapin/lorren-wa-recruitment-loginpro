...
function buildDispatchEligibilityFilter() {
...
async function findWorkerOr404(workerId) { return prisma.dispatchWorker.findUnique({ where: { id: workerId }, include: { cities: true, vacancies: true } }); }
...
export function dispatchBridgeRouter() {
  ...
  router.get('/personal/:workerId/editar', requireOps, async (req, res) => { const [worker, cities, vacancies] = await Promise.all([findWorkerOr404(req.params.workerId), loadDispatchCities(), prisma.vacancy.findMany({ select: { id: true, title: true }, orderBy: { title: 'asc' } })]); if (!worker) return res.status(404).send('Auxiliar no encontrado'); return res.render('operacionesPersonalNuevo', { cities, vacancies, worker, mode: 'edit', formAction: `/admin/operaciones/personal/${worker.id}/editar`, role: req.session?.userRole || req.userRole }); });
  router.post('/personal/:workerId/editar', requireOps, async (req, res) => { const existing = await findWorkerOr404(req.params.workerId); if (!existing) return res.status(404).send('Auxiliar no encontrado'); const workerData = buildWorkerData(req.body); if (!workerData.fullName) return res.status(400).send('Nombre requerido'); await prisma.dispatchWorker.update({ where: { id: existing.id }, data: workerData }); await replaceWorkerRelations(existing.id, req.body); return res.redirect(`/admin/operaciones/personal?message=${encodeURIComponent('Auxiliar actualizado.')}`); });
  router.post('/personal/:workerId/toggle', requireOps, async (req, res) => { const worker = await findWorkerOr404(req.params.workerId); if (!worker) return res.status(404).send('Auxiliar no encontrado'); const nextStatus = worker.operationalStatus === 'ACTIVE' ? 'INACTIVE' : 'ACTIVE'; await prisma.dispatchWorker.update({ where: { id: worker.id }, data: { operationalStatus: nextStatus } }); return res.redirect(`/admin/operaciones/personal?message=${encodeURIComponent(nextStatus === 'ACTIVE' ? 'Auxiliar reactivado.' : 'Auxiliar desactivado.')}`); });
