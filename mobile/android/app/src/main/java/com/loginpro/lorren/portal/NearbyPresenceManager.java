...
final class NearbyPresenceManager {
    ...
    private static final ConnectionOptions LOCAL_CONNECTION_OPTIONS = new ConnectionOptions.Builder()
        .setLowPower(true)
    ...
    private boolean leaderLocationReady = false;
    ...
    synchronized void startLeaderScan(JSONObject input) {
        ...
        int nextExpectedProofCount = Math.max(0, Math.min(100, input.optInt("expectedProofCount", 0)));
    ...
    synchronized void markLeaderLocationReady(String locationAttemptId) {
        if (role != Role.LEADER || !attemptId.equals(locationAttemptId)) return;
        leaderLocationReady = true;
        maybeCompleteLeaderScan();
    }
    ...
    synchronized JSONObject proofBundle() {
        ...
        try {
            ...
            result.put("expectedProofCount", expectedProofCount);
    ...
    private synchronized void maybeCompleteLeaderScan() {
        if (role != Role.LEADER || !leaderLocationReady) return;
        if (proofsByKey.size() < expectedProofCount) return;
        String completedAttemptId = attemptId;
        cancelLeaderTimers();
        client.stopAdvertising();
