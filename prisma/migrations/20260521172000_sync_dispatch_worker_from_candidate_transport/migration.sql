-- Keep DispatchWorker.transportMode in sync when Candidate.transportMode is edited.
-- The assignment board reads DispatchWorker, not Candidate, so editing a candidate must
-- also update the operational worker copy when it exists.
...
-- Backfill current operational workers from their candidate source.
...
AFTER UPDATE OF "fullName", "phone", "documentType", "documentNumber", "locality", "neighborhood", "transportMode"
ON "Candidate"
FOR EACH ROW
