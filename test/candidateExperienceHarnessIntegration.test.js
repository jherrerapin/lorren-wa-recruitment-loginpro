...
test('el flujo integral conserva edad y persiste experiencia explícita', async () => {
  ...
  try {
    ...
    const createdAt = new Date('2026-07-22T20:20:00.000Z');
    ...
    assert.equal(finalCandidate.experienceInfo, 'Sí');
    assert.equal(finalCandidate.experienceTime, '12 años');
    assert.notEqual(finalCandidate.status, 'RECHAZADO');
    assert.ok(debugTrace.persisted_fields.includes('experienceInfo'));
    assert.ok(debugTrace.persisted_fields.includes('experienceTime'));
  } finally {
    restoreAxios();
  }
});
