...
test('llegada, almuerzo y salida necesitan una evaluación facial verificada', () => {
  ...
  assert.match(portalRoute, /String\(metadata\.workerId \|\| ''\) === String\(expected\.workerId\)/);
  assert.match(portalRoute, /String\(metadata\.assignmentId \|\| ''\) === String\(expected\.assignmentId\)/);
