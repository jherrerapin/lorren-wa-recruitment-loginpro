...
test('AppUser expande identidad por nombre y correo sin retirar username histórico', () => {
  ...
  assert.match(migration, /ADD COLUMN "displayName" TEXT/);
  assert.match(migration, /ADD COLUMN "email" TEXT/);
  assert.match(migration, /ADD COLUMN "identityMigratedAt" TIMESTAMP\(3\)/);
  assert.match(migration, /CREATE UNIQUE INDEX "AppUser_email_key"/);
