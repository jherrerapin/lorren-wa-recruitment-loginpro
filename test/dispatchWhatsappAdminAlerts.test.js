...
test('los dos horarios automáticos se guardan por usuario y se interpretan en Bogotá', async () => {
  const {
  ...
  assert.deepEqual(await loadDispatchWhatsappAutomationSettings({ prismaClient, userId: 'user-test-1' }), {
    assignmentAutoSendTime: '18:30',
    pendingConfirmationAlertTime: '19:15'
  });
});

test('scheduler conserva el envío automático y el reporte horario de pendientes', async () => {
  ...
  const configEvents = [{
    ...
    metadata: { assignmentAutoSendTime: '18:30', pendingConfirmationAlertTime: '18:45' }, createdAt: new Date('2026-08-14T20:00:00.000Z')
  }];
  const runEvents = [];
  ...
  const sendAdminMessage = async ({ phone, text }) => {
    adminSends.push({ phone, text });
    return 'wamid-admin-test';
  };
  ...
  assert.match(adminSends[0].text, /Auxiliares pendientes de confirmar/);
...
test('UI conserva los dos horarios automáticos de confirmación', () => {
