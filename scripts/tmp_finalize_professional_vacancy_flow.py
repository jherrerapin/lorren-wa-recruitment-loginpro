...
# -----------------------------------------------------------------------------
# Shared public vacancy presentation
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Vacancy-first gate: shared formatter, formal public wording, explicit interest
# -----------------------------------------------------------------------------
...
function buildActiveVacancyInterestReply(vacancy = {}, inboundText = '') {
  ...
  return [
    ...
    'shared vacancy presentation block'
)
...
'''
t = once(t, old, new, 'explicit interest resolution')
write(p, t)


# -----------------------------------------------------------------------------
# Reply safety receives vacancy context
# -----------------------------------------------------------------------------
...
new = """      source: 'vacancy_first_gate',
  ...
  if (vacancyFirstGateDecision.action === VacancyFirstGateAction.ENTER_FUTURE_PROFILE_CONSENT) {"""
t = once(t, old, new, 'vacancy reply safety context')
write(p, t)


# -----------------------------------------------------------------------------
# Meta/consent path uses same formatter and respects explicit interest
# -----------------------------------------------------------------------------
...
test('Meta: confirmación con interés explícito prepara consentimiento en vez de volver a preguntar interés', () => {
  ...
  assert.match(source, /buildConsentPendingMode\(\{ cvResendRequired \}\)/);
});
'''
write(p, t)
...
t = read(p)
t = t.replace("assert.match(decision.reply, /te interesa continuar/i);", "assert.match(decision.reply, /interesa continuar|continuar con esta vacante/i);")
t = t.replace("assert.match(decision.reply, /^\\*Vacante: Líder de Operación\\*/);", "assert.match(decision.reply, /\\*Vacante: Líder de Operación\\*/);")
t = t.replace("assert.match(decision.reply, /Ciudad: Neiva/);", "assert.match(decision.reply, /\\*Ciudad:\\* Neiva/);")
t = t.replace("assert.match(decision.reply, /Zona de trabajo: Sector Las Brisas/);", "assert.match(decision.reply, /\\*Zona de trabajo:\\* Sector Las Brisas/);")
t = t.replace("assert.match(decision.reply, /\\*Funciones\\*\\nLiderar y administrar personal operativo\\./);", "assert.match(decision.reply, /\\*Funciones del cargo\\*\\nLiderar y administrar personal operativo\\./);")
...
print('professional vacancy flow patch applied')
